...
class InsertionOracle:
    ...
    selected_target: TargetObject
    new_object_pose: Affine
    ...
    def execute(self, action: Action, task: Task, scene: Scene = None):
        ...
        self.selected_object.pose = self.new_object_pose
        self.selected_target.occupied = True
        # if not test:
    ...
    def solve(self, task: Task):
        ...
        targets = [task.get_object_with_unique_id(target_id) for target_id in self.selected_objective.target_unique_ids]
        # select targets that are not occupied
        available_targets = [target for target in targets if not target.occupied]  # type: ignore
        # select random available target
        self.selected_target = random.sample(available_targets, 1)[0]
        ...
        # get a valid object pose relative to target pose
        relative_place_object_pose = self.selected_target.get_valid_poses()[0]

        # compute object pose after executing action
        self.new_object_pose = self.selected_target.pose * relative_place_object_pose

        # compute global pick and place poses
        pick_pose = self.selected_object.pose * relative_pick_gripper_pose
        # print(pick_conditioned_place_pose)
        place_pose = self.new_object_pose * relative_pick_gripper_pose
        action = Action([pick_pose, place_pose])
...
def register():
    factory.register_oracle('insertion-oracle', InsertionOracle)
